...
#ifndef KATCH_BMTCH_QUERY_H
#define KATCH_BMTCH_QUERY_H
#include <array>
#include <cassert>
#include <cmath>
#include <deque>
...
namespace katch
{
    ...
    public:
        ...
        BMTCH_Path(const NodeIterator destination, const double t_arr)
...
//        virtual int getArea() = 0;
        ...
        virtual BMTCH_Path get_path(const NodeIterator& node) const noexcept = 0;
...
//        int _end_hour;
    ...
    class BMTCHQuery :  public BMTCHQuery_Wrapper
    {
    ...
    private:
        ...
        std::vector<SearchGraph<period>> _graph_vector;
        NodeIterator _start;
    ...
    public:
                  ...
                  _upper_bound(std::numeric_limits<double>::max())
        {
            ...
            // assume the input always split the time domain equally;
        }


        bool can_ts_tch_query_answer_the_query(const double& departure_time, const double& upper_duration){
        ...
        double get_index_size(){
            double graph_size = 0;
            std::cout<<_graph_vector[0].get_index_size()<<std::endl;
            for(SearchGraph<period>& g: _graph_vector){
...
//            _t_dep = t_dep;
        ...
        BMTCH_Path get_up_down_path(NodeIterator node) const noexcept
        {
            if ( ! _context.reached(_start) ) return BMTCH_Path(_start, _t_dep);
            if ( ! _context.reached(node) ) return BMTCH_Path(_start, _t_dep);
            ...
            BMTCH_Path result(d._node_it, d._t_arr);
...
//            assert( result.dbg_check_path(*_graph, _t_dep) );
        ...
        BMTCH_Path expand_path(const BMTCH_Path& up_down_path) const noexcept
        {
            BMTCH_Path result(_start, _t_dep);
...
//            assert( result.dbg_check_path_shortcut_free(*_graph, _t_dep) );
        ...
        BMTCH_Path get_path(const NodeIterator& node) const noexcept{
