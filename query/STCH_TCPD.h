//
// Created by Bojie Shen on 1/6/21.
//

#ifndef KATCH_STCH_TCPD_H
#define KATCH_STCH_TCPD_H
#include <array>
#include <cassert>
#include <cmath>
#include <deque>
...
namespace katch
{
    ...
    private:
        static constexpr double period = 185000.0;
...
//                                 <<" Heuristic: "<<v._heuristic<<" Downward ? "<< v._is_downward<< std::endl;
    ...
    public:
        ...
        FSTCH_TCPD(std::vector<ForwardSearchGraph<&period>>&& graph)
        ...
        void load_cpd(const std::string& filename){
            _cpd = CPD();
            _bw_cpd = BW_CPD();
            string main_name = filename.substr(0,filename.find_last_of("."));
            main_name = main_name.substr(0, main_name.find_last_of("_"));
            for(int i = 0; i < 24 ; i ++){
                string cpd_name = main_name + "_" + to_string(i)  + "_min.fw_htch_cpd";
                string bw_cpd_name = main_name + "_" + to_string(i)  + "_min.bw_htch_cpd";
                ...
                string bw_cpd_mapper_name = main_name + "_" + to_string(i)  + "_min.bw_htch_mapper";
                vector<unsigned> bw_mapper = load_vector<unsigned>(bw_cpd_mapper_name );

                assert(cur_fw_cpd.node_count() ==_graph_vector[i].get_n_nodes());
                assert(cur_bw_cpd.node_count() == _graph_vector[i].get_n_nodes());
            ...
            string mapper_name = main_name  + ".fw_htch_mapper";
        ...
        double one_to_one(const NodeIterator start, const NodeIterator destination, const double t_dep)
        {
            ...
            _t_dep = katch::util::modulo(t_dep,36000.0);
            double _t_base = t_dep - _t_dep;

            _graph = &_graph_vector[katch::util::modulo((int)(t_dep/36000.0),24)];
