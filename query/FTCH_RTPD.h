//
// Created by Bojie Shen on 23/10/21.
//

#ifndef KATCH_REV_TCH_CPD_SEARCH_H
#define KATCH_REV_TCH_CPD_SEARCH_H
#include <array>
#include <cassert>
#include <cmath>
#include <deque>
...
namespace katch
{
    ...
    private:
        ...
        double get_lower_bound(const unsigned& start){
            ...
            const unsigned char& fm = _cpd.get_first_move(start);
...
//        }
    ...
    public:
        ...
        unsigned long query_index;
        using Graph = ForwardSearchGraph<&period> ;

        FTCH_RTPD(ForwardSearchGraph<&period> && graph)
...
//            _cpd_free_flow = load_vector<double>(filename.substr(0,filename.find_last_of(".")) + ".fw_tch_free_flow");
        ...
        void load_target_row(const double t_dep){
            _cpd.load_to_cache(query_index%10000);
            query_index ++;
        }
...
//            _cpd.fetch_target_row(query_index%10000);
//            query_index ++;
//            load_target_row();
...
#endif //KATCH_REV_TCH_CPD_SEARCH_H