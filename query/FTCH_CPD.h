...
#ifndef KATCH_FTCH_CPD_H
#define KATCH_FTCH_CPD_H
#include <array>
#include <cassert>
#include <cmath>
#include <deque>
...
//                if(_start == 6002 && _destination == 24036){
//                    if(u_it == 6005){
//                        bool a =0;
//                    }
//                }
...
//                                 <<" Heuristic: "<<v._heuristic<<" Downward ? "<< v._is_downward<< std::endl;
                            ...
                            v._is_downward = false;
                            //CH-CPD is a inconsistent heuristic, because the lower bound of shortcuts is larger than the lower bound of original arcs.
                            //although CPD is a consistent heuristic, but someteims t_arr_v_new could == v._t_arr, so keep this open, doesnt affect too much performance
                            if ( _context.pq_contains(v) )
                                _context.pq_decrease(v, t_arr_v_new + v._heuristic);
                            else
                                _context.pq_re_insert(v, t_arr_v_new + v._heuristic);
...
//        }
    ...
    public:
        ...
        FTCH_CPD(ForwardSearchGraph<&period> && graph)
        ...
        void load_cpd(const std::string& filename){
            ...
            _cpd_mapper = load_vector<unsigned>(name.substr(0,name.find_last_of("_"))+".ori_fw_tch_mapper");
...
//            unsigned max_size  =0 ;
//            for(unsigned i = 0; i < _cpd_bw_mapper.size(); i++){
//                if( max_size < _bw_cpd.get_row_size(i)){
//                    max_size = _bw_cpd.get_row_size(i);
//                }
//                total_size = total_size + _bw_cpd.get_row_size(i);
//            }
//            std::cout<< total_size /  _cpd_bw_mapper.size() << std::endl;
