//
// Created by Bojie Shen on 22/11/21.
//

#ifndef KATCH_BTCH_L_H
#define KATCH_BTCH_L_H
#include <array>
#include <cassert>
#include <cmath>
#include <deque>
...
//            return 0;
            ...
            for ( const auto& search_node_id : _candidates )
            {
                ...
                if ( u._t_arr + u._interval.get_lower() <= _upper_bound + 0.01 )
                    if ( ! _context.pq_contains(u, FORWARD) ){
                        _context.pq_re_insert(u, u._t_arr + u._interval.get_lower() , FORWARD);
            ...
            while ( ! _context.pq_empty(FORWARD) )
            {
                ...
                for ( auto& pred : u._bw_predecessors )
                {
                    ...
                    if ( v._t_arr == std::numeric_limits<double>::max() )
                    {
                        ...
                        v._fw_h = v._interval.get_lower() ;
                        _context.pq_re_insert(v, t_arr_v_new + v._fw_h, FORWARD);
                        number_of_nodes_generated++;
                    ...
                    else
                    {
                        ...
                        if ( _context.pq_contains(v, FORWARD) )
                            _context.pq_decrease(v, t_arr_v_new + v._fw_h, FORWARD);
                        else{
                            v._fw_h = v._interval.get_lower() ;
                            _context.pq_re_insert(v, t_arr_v_new + v._fw_h, FORWARD);
                        }
        ...
        void bidirectional_search()
        {
            ...
            double fw_h = get_lower_bound(_start,_destination);
            SearchNode& s = _context.insert(_start, _t_dep + fw_h, FORWARD);
            number_of_nodes_generated++;
            s._t_arr = _t_dep;
            s._fw_h = fw_h;

            double bw_h = fw_h;
            SearchNode& d = _context.insert(_destination, 0.0 + bw_h, BACKWARD);
            number_of_nodes_generated++;
            ...
            while ( ! _context.pq_empty(FORWARD) || ! _context.pq_empty(BACKWARD) )
            {
                ...
                if ( direction == FORWARD )
                {
                    ...
                    number_of_nodes_expanded++;
                    if ( _upper_bound != std::numeric_limits<double>::max() && u._t_arr + u._fw_h > _upper_bound + 0.001)
                    {
                    ...
                    for ( EdgeIterator e = _graph.edges_begin(u_it) ; e != _graph.edges_end(u_it) ; ++e )
                    {
                        ...
                        if ( ! _context.reached(v_it) )
                        {
                            const double t_arr_v_new = _graph.get_ttf(e).eval(t_arr_u) + t_arr_u;
                            number_of_TD_calculation ++;
                            fw_h = get_lower_bound(v_it,_destination);
                            SearchNode& v = _context.insert(v_it, t_arr_v_new + fw_h, FORWARD);
                            v._t_arr = t_arr_v_new;
                            //update _fw_h;
                            v._fw_h = fw_h;
                        ...
                        else
                        {
                            ...
                            if ( _context.pq_contains(v, FORWARD) ){
                                ...
                                v._fw_h = get_lower_bound(v_it,_destination);
                                _context.pq_re_insert(v, t_arr_v_new + v._fw_h, FORWARD);
                            }
                ...
                else
                {
                    ...
                    number_of_nodes_expanded++;
                    if ( _upper_bound != std::numeric_limits<double>::max() && u._interval.get_lower() + u._bw_h >  _upper_bound - _t_dep + 0.001 )
                    {
                    ...
                    for ( EdgeIterator e = _graph.edges_begin(u_it) ; e != _graph.edges_end(u_it) ; ++e )
                    {
                        ...
                        if ( ! _context.reached(v_it) )
                        {
                            // compute heuristic in a reverse way;
                            bw_h = get_lower_bound(_start,v_it);
                            SearchNode& v = _context.insert(v_it, interval_v_new.get_lower() + bw_h, BACKWARD);
                        ...
                        else
                        {
                            ...
                            if ( _context.pq_contains(v, BACKWARD) )
                                _context.pq_decrease(v, v._interval.get_lower() + v._bw_h , BACKWARD);
                            else{
                                v._bw_h = get_lower_bound(_start,v_it);
                                _context.pq_re_insert(v, v._interval.get_lower() + v._bw_h , BACKWARD);
                            }
                            number_of_nodes_generated++;
                        }
                    }
    ...
    public:
        ...
        BTCH_L(SearchGraph<&period> && graph)
        ...
        Path get_path(NodeIterator node) const noexcept{
            const katch::BTCH_L::Path up_down_path = get_up_down_path(node);
