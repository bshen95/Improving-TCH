...
public:
    ...
    {
        if ( _is_constant ){
            return std::vector<Point>(1, Point (time_period.first,_constant_value));
        }
        assert( _ttf_impl_ptr );
        return _ttf_impl_ptr-> get_turning_points(time_period);
    }
    ...
    double get_average(int mins) const noexcept
    {
        ...
        assert( _ttf_impl_ptr );
        return _ttf_impl_ptr->get_average(mins);
    }

    double get_hourly_min(unsigned hourly_index) const noexcept
    {
        if ( _is_constant ) return _constant_value;

        assert( _ttf_impl_ptr );
        return _ttf_impl_ptr->get_hourly_min(hourly_index);
    }

    double get_hourly_max(unsigned hourly_index) const noexcept
    {
        if ( _is_constant ) return _constant_value;

        assert( _ttf_impl_ptr );
        return _ttf_impl_ptr->get_hourly_max(hourly_index);
    }
...
//        assert( ! dbg_is_a_hidden_constant_function() );
    ...
    template <typename PointIterator>
    TTFWrapper(PointIterator points_begin, PointIterator points_end, double hourly_time_period) noexcept
            : _ttf(new TTFImpl(points_begin, points_end, hourly_time_period))
    {
        assert( points_end >= points_begin );
...
//        assert( ! dbg_is_a_hidden_constant_function() );
    ...
    double get_min(std::pair<double,double> time_period) const noexcept { return _ttf.get_min(time_period); }
    double get_min() const noexcept { return _ttf.get_min(); }
    double get_max() const noexcept { return _ttf.get_max(); }
    double get_average(int mins) const noexcept { return _ttf.get_average(mins); }
