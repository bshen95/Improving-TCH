...
namespace katch
{
    ...
    private:
        ...
        void set_up_hourly_min_max( double _time_period)
        {
            _hourly_time_period = _time_period;
            for ( auto it = _points.begin() ; it != _points.end() ; ++it )
            {
                for(unsigned i = 0; i < 24; i++){
                    double time_min = i * 36000.0;
                    double time_max = i * 36000.0 + _time_period;

                    if(time_max > 864000.0){
                        if( time_min <= it->x && it->x <= 864000.0){
                            _hourly_min[i] = std::min(_hourly_min[i], it->y );
                            _hourly_max[i] = std::max(_hourly_max[i], it->y );
                        }
                        if( 0 <= it->x && it->x <= time_max -864000.0){
                            _hourly_min[i] = std::min(_hourly_min[i], it->y );
                            _hourly_max[i] = std::max(_hourly_max[i], it->y );
                        }

                    }else{
                        if( time_min <= it->x && it->x <= time_max){
                            _hourly_min[i] = std::min(_hourly_min[i], it->y );
                            _hourly_max[i] = std::max(_hourly_max[i], it->y );
                        }
                    }
            ...
            for(unsigned i = 0; i < 24; i++){
                double time_min = i * 36000.0;
                double time_max = i * 36000.0 + _time_period;
                double start_value = eval(time_min);
                double end_value = eval(time_max);
                _hourly_min[i] = std::min(_hourly_min[i], start_value );
                _hourly_max[i] = std::max(_hourly_max[i], start_value );
                _hourly_min[i] = std::min(_hourly_min[i], end_value );
                _hourly_max[i] = std::max(_hourly_max[i], end_value );
            }
//        if(_hourly_min[0] != _min){
//            std::cout<<"weird"<<std::endl;
//        }
//        if(_hourly_max[0] != _max){
//            std::cout<<"weird"<<std::endl;
//        }
        }
        ...
        double _min;
        double _max;
        double _hourly_time_period;
        std::vector<double> _hourly_min;
        std::vector<double> _hourly_max;
        ...
        PwlTTF(std::vector<Point>&& points) noexcept
                : _bucket_shift( compute_bucket_shift(points.begin(), points.end()) ),
                  ...
                  _hourly_min(24, std::numeric_limits<double>::max() ),
                  _hourly_max(24, std::numeric_limits<double>::min() )
        {
    ...
    public:
        ...
        template <typename Iterator>
        PwlTTF(Iterator points_begin, Iterator points_end, double hourly_time_period) noexcept
                : _bucket_shift(0),
                  _points(),
                  _buckets(),
                  _min( std::numeric_limits<double>::max() ),
                  ...
                  _hourly_max(24, std::numeric_limits<double>::min() )
        {
            ...
            set_up_buckets_and_min_max();
            set_up_hourly_min_max(hourly_time_period);
            assert( dbg_check_points_valid() );
        }
        ...
        size_t get_n_points(std::pair<double,double>time_periods) const noexcept
        {
            size_t size =0;
            for(Point p: _points){
                if(ge(p.x,time_periods.first) && lt(p.x, time_periods.second)){
                    size++;
                }
        ...
        bool is_constant(std::pair<double,double>time_periods) const noexcept
        {
            return eq(eval(time_periods.first),eval(time_periods.second));
        }

        bool is_constant_start() const noexcept
        {
            return _points[0].y == _points[1].y;
        }
        bool is_constant_end() const noexcept
        {
            return _points[_points.size()-1].y == _points[_points.size()-2].y;
        }
        ...
        double get_hourly_min(unsigned hour_id) const noexcept
        {
            assert( this );

            return _hourly_min[hour_id];
        }

        double get_hourly_max(unsigned hour_id) const noexcept
        {
            assert( this );

            return _hourly_max[hour_id];
        }

        size_t get_n_bucket() const noexcept
        {
            return _buckets.size();
        }

        double get_min(std::pair<double, double> time_period) const noexcept
        {
            assert( this );
            ...
            assert(time_period.first < time_period.second);
            double period_min = std::min(eval(time_period.first),eval(time_period.second));
            double s_x = util::modulo(time_period.first,PERIOD());
            double t_x = util::modulo(time_period.second,PERIOD());

            if(s_x < t_x ){
                for( const Point& p : _points){
                    if(p.x > s_x  && p.x < t_x )
                        period_min = std::min(period_min,p.y);
                }
            }else{
                // t_x < s_x
                for( const Point& p : _points){
            ...
            assert(le(_min,period_min));
            return period_min;
        }


        std::vector<Point> get_turning_points(std::pair<double,double> time_period) const noexcept
        {
            assert(time_period.second - time_period.first < PERIOD());
            double s_x = util::modulo(time_period.first,PERIOD());
            double s_base = time_period.first - s_x;
            double t_x = util::modulo(time_period.second,PERIOD());
            double t_base = time_period.second - t_x;
            std::vector<Point> turning_points;
            turning_points.emplace_back(Point(time_period.first,eval(time_period.first)));
            if(s_x < t_x){
                for( const Point& p : _points){
                    if(p.x > s_x && p.x < t_x  )
                        turning_points.emplace_back(Point(p.x + s_base,p.y));
                }
            }else {
                for (const Point &p : _points) {
                    if (p.x > s_x)
                        turning_points.emplace_back(Point(p.x + s_base, p.y));
                    if (p.x < t_x)
                        turning_points.emplace_back(Point(p.x + t_base, p.y));
                }
            }
            std::sort(turning_points.begin(),turning_points.end(),[](const Point& lhs, const Point& rhs)
            {
                return lhs.x < rhs.x;
            });
            turning_points.emplace_back(Point(time_period.second, eval(time_period.second)));

            // refine the constant case
            bool is_constant = true;
            for(int i =0;i<turning_points.size()-1;i++){
                if(turning_points[i].y != turning_points[i+1].y){
                    is_constant =false;
                    break;
                }
            }
            if(is_constant){
                std::vector<Point> tmp_turning_points {turning_points[0]};
                turning_points = tmp_turning_points;
            }
        ...
        double get_average(int mins) const noexcept
        {
            double _sum = 0;
            int num =0 ;
            for(int i = 0; i < PERIOD(); i += mins*600){
                _sum += eval(i);
                num++;
            }
            return _sum/num;
        ...
        friend inline PwlTTF* merge(PwlTTF const * const f_ptr, PwlTTF const * const g_ptr, UndercutDescriptor& descr) noexcept
        {
            ...
            // little bit accuracy issue but should be fine.
