...
#ifndef KATCH_REV_BW_CPD_H
#define KATCH_REV_BW_CPD_H
#pragma once
#include <vector>
#include <set>
#include <algorithm>
...
namespace katch {
    ...
    public:
        ...
        void append_rows(const REV_BW_CPD&other);

        void append_rows(const REV_BW_CPD &other, unsigned row_id);
        ...
        bool get_reachability(unsigned source_node, unsigned target_node) const {
            ...
            return *binary_find_last_true(
                    entry.begin() + begin[target_node],
                    entry.begin() + begin[target_node + 1],
                    [=](unsigned x) { return x <= source_node; }
            ) & 0001;
        }
        ...
        friend bool operator==(const REV_BW_CPD &l, const REV_BW_CPD &r) {
