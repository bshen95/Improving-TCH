#ifndef KATCH_ID_QUEUE_H
#define KATCH_ID_QUEUE_H
...
public:
    ...
    bool contains_id(katch::NodeIterator id){
        assert(id < id_count());
    ...
    double get_key(katch::NodeIterator id)const{
        assert(id < id_count());
...
//        std::swap(heap[0].g,  heap[heap_size].g);
    ...
    bool decrease_key(IDKeyPair p){
        ...
        if(katch::gt(heap[pos].key , p.key)){
            heap[pos].key = p.key;
            move_up_in_tree(pos);
            return true;
        } else {
    ...
    bool increase_key(IDKeyPair p){
        ...
        if(katch::lt(heap[pos].key,p.key)){
            heap[pos].key = p.key;
...
private:
    void move_up_in_tree(unsigned pos){
        while(pos != 0){
            ...
            if(katch::gt(heap[parent].key , heap[pos].key)){
    ...
    void move_down_in_tree(unsigned pos){
        for(;;){
            ...
            for(unsigned c = first_child+1; c < std::min(tree_arity*pos+tree_arity+1, heap_size); ++c){
                if(katch::gt(heap[smallest_child].key ,heap[c].key)){
                    smallest_child = c;
                }
            }

            if(katch::ge(heap[smallest_child].key , heap[pos].key))
